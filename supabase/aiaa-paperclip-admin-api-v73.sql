...
create table if not exists public.aiaa_email_notification_queue (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists aiaa_email_notification_queue_status_idx on public.aiaa_email_notification_queue(status, created_at desc);
create index if not exists aiaa_email_notification_queue_user_idx on public.aiaa_email_notification_queue(user_id, created_at desc);

alter table public.aiaa_reviewer_actions enable row level security;
alter table public.aiaa_email_notification_queue enable row level security;
...
create or replace function public.aiaa_resolve_notification_email(target_user_id uuid, target_contact_email text)
returns text
language plpgsql
security definer
as $$
declare
  account_email text;
begin
  if coalesce(nullif(target_contact_email, ''), '') <> '' then
    return target_contact_email;
  end if;
  ...
  return coalesce(account_email, '');
end;
$$;

create or replace function public.aiaa_queue_email_notification(
...
declare
  resolved_email text;
begin
  resolved_email := public.aiaa_resolve_notification_email(target_user_id, target_contact_email);

  if coalesce(resolved_email, '') = '' then
    return;
  end if;

  insert into public.aiaa_email_notification_queue(
    application_id,
    user_id,
    recipient_email,
    type,
...
begin
  if new.precheck_status = 'approved' and coalesce(old.precheck_status, '') is distinct from 'approved' then
    ...
    new.status = case when new.status = 'rejected' then 'submitted' else coalesce(nullif(new.status, ''), 'submitted') end;
    new.stage = 'Exam';
  ...
  if new.precheck_status = 'rejected' and coalesce(old.precheck_status, '') is distinct from 'rejected' then
    new.exam_access_status = 'locked';
    new.status = 'rejected';
    new.stage = 'Application';
  end if;
  ...
  if new.review_status = 'approved' and coalesce(old.review_status, '') is distinct from 'approved' then
    ...
    if new.certificate_status <> 'issued' then
      new.certificate_status = 'ready';
    end if;
    new.stage = case when new.certificate_status = 'issued' then 'Ranking' else 'Certificate' end;
  end if;

  if new.review_status = 'revision_required' and coalesce(old.review_status, '') is distinct from 'revision_required' then
    new.review_decision = 'revision_required';
    new.reviewed_at = coalesce(new.reviewed_at, now());
  ...
  if new.review_status = 'rejected' and coalesce(old.review_status, '') is distinct from 'rejected' then
  ...
  if new.certificate_status = 'issued' and coalesce(old.certificate_status, '') is distinct from 'issued' then
    ...
    if coalesce(new.certificate_id, '') = '' then
      new.certificate_id = concat('AIAA-L', new.target_level, '-', upper(substr(new.id::text, 1, 8)));
    end if;
...
begin
  if tg_op = 'INSERT' then
    ...
    perform public.aiaa_queue_email_notification(
      new.id,
      new.user_id,
      new.contact_email,
      'application_received',
  ...
  if tg_op = 'UPDATE' then
    if coalesce(old.precheck_status, '') is distinct from coalesce(new.precheck_status, '') then
      if new.precheck_status = 'approved' then
        ...
        values (
          ...
          'exam_unlocked',
          'Exam unlocked',
          'Your application passed precheck. The exam is now available in your member workspace.',
          '/member/exam',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
        ...
        perform public.aiaa_queue_email_notification(
          ...
          'precheck_approved',
          'Precheck approved',
          'Your AIAA certification application passed precheck.',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
        ...
        perform public.aiaa_queue_email_notification(
          ...
          'exam_unlocked',
          'Exam unlocked',
          'Your application passed precheck. The exam is now unlocked in your member workspace.',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
        ...
        values (
          ...
          coalesce(nullif(new.precheck_note, ''), 'The application did not meet the current requirements.'),
          '/member/applications',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
    ...
    if coalesce(old.exam_status, '') is distinct from coalesce(new.exam_status, '') and new.exam_status = 'submitted' then
      ...
      perform public.aiaa_queue_email_notification(
        ...
        'exam_submitted',
        'Exam submitted',
        'Your exam was submitted and locked. The application is now waiting for reviewer assessment.',
        jsonb_build_object('application_id', new.id, 'level', new.target_level, 'score_percent', new.exam_score_percent, 'scoring_status', new.exam_scoring_status)
      );
    end if;
    ...
    if coalesce(old.review_status, '') is distinct from coalesce(new.review_status, '') then
      if new.review_status = 'approved' then
        ...
        values (
          ...
          coalesce(nullif(new.review_note, ''), 'Your certification application passed review. Certificate issuance is the next step.'),
          '/member/applications',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
        ...
        values (
          ...
          'Revision required',
          coalesce(nullif(new.review_note, ''), 'Reviewers requested changes after exam assessment.'),
          '/member/applications',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
        ...
        values (
          ...
          'review_rejected',
          'Review rejected',
          coalesce(nullif(new.review_note, ''), 'The application did not pass final review.'),
          '/member/applications',
          jsonb_build_object('application_id', new.id, 'level', new.target_level)
        );
    ...
    if coalesce(old.certificate_status, '') is distinct from coalesce(new.certificate_status, '') and new.certificate_status = 'issued' then
      ...
      values (
        ...
        'certificate_issued',
        'Certificate issued',
        'Your AIAA certificate has been issued. Your public profile and ranking eligibility can now update.',
        '/member/applications',
        jsonb_build_object('application_id', new.id, 'level', new.target_level, 'certificate_id', new.certificate_id)
      );
      ...
      if new.target_level < 5 then
        insert into public.aiaa_notifications(user_id, type, title, message, link, metadata)
        values (
          new.user_id,
          'next_level_unlocked',
          'Next level unlocked',
          concat('You can now apply for Level ', new.target_level + 1, '.'),
          '/apply/agent',
          jsonb_build_object('application_id', new.id, 'current_level', new.target_level, 'next_level', new.target_level + 1)
        );
