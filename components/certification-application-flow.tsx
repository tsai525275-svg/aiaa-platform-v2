...
function FlowStep({ name, index, activeIndex, approved }: { name: string; index: number; activeIndex: number; approved: boolean }) {
  ...
  const label = approved || index < activeIndex ? "完成" : index === activeIndex ? "目前" : "等待";
    ...
    <div className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-3xl font-semibold tracking-[-0.05em] text-neutral-950">{application.agent_name || "AIAA 認證申請"}</h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-neutral-600">{application.evidence_summary || "尚未填寫證據摘要。"}</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <Info label="考試狀態" value={application.exam_status || "not_started"} />
        <Info label="審核狀態" value={application.reviewer_status || "waiting"} />
        <Info label="證書" value={application.certificate_id || "未核發"} />
      </div>
    </div>
  );
}

function AuthGate({ copy = "登入後才能提交和追蹤 AIAA 認證申請。" }: { copy?: string }) {
  const loginUrl = getLoginUrl(getNextPath());
  return (
    <Section compact eyebrow="Member gate" title="請先登入會員。" copy={copy}>
      <div className="flex flex-wrap gap-3">
        <Link href={loginUrl} className="aiaa-button-dark">登入後申請</Link>
...
function useCertificationFlow() {
  ...
  async function reload() {
    if (!isSupabaseAuthConfigured()) {
      setState({ ...emptyState, loading: false, error: "Supabase 尚未設定。" });
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "無法讀取申請流程。";
...
export function CertificationApplicationForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (nextLevel > 1 && approvedLevel < nextLevel - 1) {
      setMessage(`你必須先通過 ${levelName(nextLevel - 1)}，才能申請 ${levelName(nextLevel)}。`);
      return;
    }

    if (!form.agentName.trim() || !form.githubRepo.trim() || !form.readmeUrl.trim() || !form.evidenceSummary.trim()) {
      setMessage("代理人名稱、GitHub Repo、README URL、證據摘要都要填。Demo 和影片可先空著。");
      return;
    }
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "提交申請失敗。請重新登入後再試一次。");
    } finally {
      setSubmitting(false);
    }
  }

  if (loading) return <Section compact><p className="text-sm text-neutral-600">正在讀取會員狀態。</p></Section>;
  if (authRequired) return <AuthGate copy="登入後，這裡會直接顯示提交申請表，不會再顯示建立帳戶流程。" />;
  if (error) return <Section compact><p className="text-sm text-rose-700">{error}</p></Section>;

  if (activeApplication) {
    return (
      <Section compact eyebrow="Active application" title="你已有一個進行中的申請。" copy="完成目前的申請、考試、審核、證書和排名流程後，才能開下一級。">
        <ApplicationCard application={activeApplication} />
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/member/applications" className="aiaa-button-dark">查看申請進度</Link>
          <Link href="/member/exam" className="aiaa-button-light">進入考試</Link>
        </div>
      </Section>
    );
  }
  ...
  return (
    <Section compact eyebrow="Application form" title={`提交 ${levelName(nextLevel)} 申請。`} copy="此表單會寫入 Supabase 申請資料表。提交後只代表進入申請流程，不會自動通過，也不會自動核發證書。">
      <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="space-y-6">
          <div className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)]">
            ...
            <h3 className="mt-4 text-2xl font-semibold tracking-[-0.05em] text-neutral-950">目前要提交的申請。</h3>
            <p className="mt-3 text-sm leading-7 text-neutral-600">新會員從 Level 1 開始。通過前，不會顯示已認證，也不會解鎖下一級。</p>
          </div>
          <ThinTable
            headers={["步驟", "狀態"]}
            rows={[
        ...
        <div className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <Field label="GitHub Repo" hint="必填。可填公開 Repo 或審核用連結。">
            ...
            <Field label="README URL" hint="必填。說明安裝、流程、測試方式。">
            ...
            <div className="md:col-span-2">
              <Field label="證據摘要" hint="必填。寫清楚代理人會做什麼，怎麼運作，有哪些 API、工具、錯誤處理和人工審核點。">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-6">
            <button type="submit" disabled={submitting} className="aiaa-button-dark disabled:cursor-not-allowed disabled:opacity-50">{submitting ? "提交中" : "提交申請"}</button>
            <Link href="/member/applications" className="aiaa-button-light">查看申請進度</Link>
...
export function MemberApplicationsTracker() {
  ...
  if (loading) return <Section compact><p className="text-sm text-neutral-600">正在讀取申請紀錄。</p></Section>;
  if (authRequired) return <AuthGate copy="登入後才能查看自己的申請、考試、審核和證書狀態。" />;
  if (error) return <Section compact><p className="text-sm text-rose-700">{error}</p></Section>;

  if (!applications.length) {
    return (
      <Section compact eyebrow="Application tracker" title="尚未提交 Level 1 申請。" copy="新會員不會自動獲得 Level 1。先提交申請，再完成考試，然後等待審核。">
        <Link href="/apply/agent" className="aiaa-button-dark">提交 Level 1 申請</Link>
      </Section>
    );
  }
  ...
  return (
    <>
      <Section compact eyebrow="Certification status" title={approvedLevel ? `${levelName(approvedLevel)} 已通過。` : "尚未有通過的認證。"} copy="只有審核通過且有證書 ID 的紀錄，才會顯示為已認證。">
        <div className="grid gap-4 md:grid-cols-3">
          <Info label="已通過級別" value={approvedLevel ? levelName(approvedLevel) : "None"} />
          <Info label="目前申請" value={activeApplication ? levelName(activeApplication.target_level) : "沒有進行中的申請"} />
          <Info label="下一步" value={activeApplication ? stageLabel(activeApplication.stage) : "提交下一級申請"} />
        </div>
      </Section>
      <Section compact eyebrow="Applications" title="你的申請紀錄。" copy="這裡讀取 Supabase 真實資料，不顯示假的通過狀態。">
...
export function MemberExamWorkspace() {
  ...
  async function startExam() {
    ...
    try {
      ...
      await reload();
      setMessage("考試已開始。完成所有回答後送出審核。");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "無法開始考試。");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function submitExam(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await reload();
      setMessage("考試已送出。現在進入審核，不會自動核發證書。");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "送出考試失敗。");
    } finally {
      setSaving(false);
    }
  }

  if (loading) return <Section compact><p className="text-sm text-neutral-600">正在讀取考試資料。</p></Section>;
  if (authRequired) return <AuthGate copy="登入後才能進入你的考試工作區。" />;
  if (error) return <Section compact><p className="text-sm text-rose-700">{error}</p></Section>;

  if (!activeApplication) {
    return (
      <Section compact eyebrow="Exam" title="目前沒有考試。" copy="先提交 Level 1 申請，申請紀錄建立後才會出現考試。">
        <Link href="/apply/agent" className="aiaa-button-dark">提交 Level 1 申請</Link>
      </Section>
    );
  }
  ...
  if (activeApplication.status === "under_review") {
    return (
      <Section compact eyebrow="Exam submitted" title="已送出，等待審核。" copy="審核員必須檢查考試和證據後，才會核發證書。">
        <ApplicationCard application={activeApplication} />
      </Section>
    );
  }
  ...
  return (
    <Section compact eyebrow="Exam workspace" title={`${levelName(activeApplication.target_level)} 考試。`} copy="考試送出後，申請會進入審核。這不會自動通過，也不會自動核發證書。">
      <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="space-y-6">
          <ApplicationCard application={activeApplication} />
          {activeApplication.stage === "Application" ? <button type="button" onClick={startExam} disabled={saving} className="aiaa-button-dark disabled:cursor-not-allowed disabled:opacity-50">開始考試</button> : null}
        </div>
        <form onSubmit={submitExam} className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">
          <div className="grid gap-5">
            <Field label="架構說明">
              <textarea value={answers.architecture} onChange={(event) => update("architecture", event.target.value)} className={textareaClass} placeholder="說明 Agent 的工作流、執行環境、狀態管理和輸入輸出。" />
            </Field>
            <Field label="工具呼叫證明">
            ...
            <Field label="錯誤處理和重試邏輯">
              <textarea value={answers.errorHandling} onChange={(event) => update("errorHandling", event.target.value)} className={textareaClass} placeholder="說明錯誤訊息、重試、記錄、回復規則和人工介入點。" />
            </Field>
            <Field label="審核執行說明">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-6">
            <button type="submit" disabled={saving || activeApplication.stage === "Application"} className="aiaa-button-dark disabled:cursor-not-allowed disabled:opacity-50">送出考試並進入審核</button>
            <Link href="/member/applications" className="aiaa-button-light">查看申請進度</Link>
            {message ? <span className="text-sm leading-6 text-rose-700">{message}</span> : null}
          </div>
        </form>
...
export function MemberApplicationSnapshot() {
  ...
  if (loading) return <section className="border border-slate-200 bg-white p-6 text-sm text-neutral-600 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">正在讀取認證狀態。</section>;
  if (authRequired) return <section className="border border-slate-200 bg-white p-6 text-sm text-neutral-600 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">請重新登入以查看認證狀態。</section>;
  ...
  return (
    <>
      <section className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">
        ...
        <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-neutral-950">{approvedLevel ? `${levelName(approvedLevel)} 已通過。` : "尚未有通過的認證。"}</h2>
        <p className="mt-4 text-sm leading-7 text-neutral-600">{approvedLevel ? "通過的認證會顯示在會員資料、註冊紀錄、證書和排名資格中。" : "新會員不會自動得到 Level 1。必須提交申請、完成考試、通過審核後才會顯示。"}</p>
        <div className="mt-5">
          <Info label="已通過級別" value={approvedLevel ? levelName(approvedLevel) : "None"} />
          <Info label="證書" value={applications.find((item) => item.target_level === approvedLevel)?.certificate_id || "未核發"} />
          <Info label="排名資格" value={approvedLevel ? "等待排名審核" : "未符合"} />
        </div>
        <Link href={activeApplication ? "/member/applications" : "/apply/agent"} className="aiaa-button-dark mt-6">{activeApplication ? "查看申請進度" : "提交 Level 1 申請"}</Link>
      </section>

      <section className="border border-slate-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.055)] lg:p-7">
        <p className="text-[0.72rem] font-semibold uppercase tracking-[0.28em] text-neutral-500">Current application</p>
        <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-neutral-950">{activeApplication ? `${levelName(activeApplication.target_level)}，${statusLabel(activeApplication.status)}。` : "Level 1 申請尚未提交。"}</h2>
        <p className="mt-4 text-sm leading-7 text-neutral-600">{activeApplication ? "此狀態來自你的 Supabase 申請紀錄。" : "先提交 Level 1 申請。提交後，這裡會顯示申請、考試、審核、證書和排名階段。"}</p>
